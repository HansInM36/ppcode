...
# wrf_files, interp_files, camx_interp_fname, dynamic_driver_file, times_sec, dimensions, z_levels, z_levels_stag, ztop, z_soil_levels, dx, dy, lon_center, lat_center, rad_times_proc, rad_values_proc, sma, nested_domain = wrf_files, interp_files, camx_interp_fname, dynamic_driver_file, times_sec, dimensions, z_levels, z_levels_stag, ztop, z_soil_levels, dx, dy, cent_lon, cent_lat, rad_times_proc, rad_values_proc, soil_moisture_adjust, nested_domain


    print('Processing interpolated files to dynamic driver')
    # dimension of the time coordinate
    dimtimes = len(times_sec) + 1 # !!!!!
    # other coordinates
    ...
    try:
        ...
        areas_zb = dx*dy
        # sum of areas of four vertical boundary and the top boundary
        ...
        # print(times_sec, dimtimes, '1!!!!!!!!!!!!!!!111111111111')
        _val_times[:] = np.array([0, 100000.0]) # !!!!
        ...
        if not nested_domain:
            ...
            for ts in [0,1]: # !!!!
                ...
                nc_wrf = netCDF4.Dataset(wrf_files[0], 'r') # !!!!
                try:
                    ug, vg = palm_wrf_gw(nc_wrf, lon_center, lat_center, z_levels) # bug3 # debug3
                    # ug, vg = -3.941, 0.827 # debug3
        ...
        # # add another time step !!!!
        # outfile.variables['time'] = np.concatenate((outfile.variables['time'], outfile.variables['time']),axis=0)
        # outfile.variables['time'][1] = 10000.0
        #
        # varList = list(outfile.variables)
        # lsvarList = [i for i in varList if 'ls_' in i]
        # for var in lsvarList:
        #     print(var)
        #     outfile.variables[var] = np.vstack((outfile.variables[var][:],outfile.variables[var][:]))
