...
xInd_start, xInd_end = 39, 90
yInd_start, yInd_end = 39, 90
...
for zInd in range(zNum):
    ...
    # calculate average var field
    var_av = []
    for tInd in range(tNum):
        var_av.append(varSeq[tInd,zInd])
    var_av = np.average(np.array(var_av), axis=0)

    plotDataList.append((xSeq, ySeq, var_av))

    # plotDataList.append((xSeq, ySeq, varSeq[-1,zInd]))
...
cbreso = 100 # resolution of colorbar
x_ = plotDataList[zInd][0]
y_ = plotDataList[zInd][1]
v_ = plotDataList[zInd][2]
# v_ = v_ - v_.mean()
CS = axs.contourf(x_, y_, v_, cbreso, cmap='jet')
cbar = plt.colorbar(CS, ax=axs, shrink=1.0)
cbar.ax.set_ylabel(varName + ' (m/s)', fontsize=12)
plt.ylabel('y (m)', fontsize=12)
plt.xlabel('x (m)', fontsize=12)
plt.text(0.6, 1.02, 'h = ' + str(int(HList[zInd])) + 'm', transform=axs.transAxes, fontsize=12)
saveName = varName + '_contour_av_' + str(int(HList[zInd])) + '.png'
plt.savefig(ppDir + '/' + saveName, bbox_inches='tight')
plt.show()
