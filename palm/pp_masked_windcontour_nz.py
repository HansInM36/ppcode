...
varseq = np.power(np.power(varseq1,2) + np.power(varseq2,2),0.5)

### plot
fig, axs = plt.subplots(figsize=(8,8))
cbreso = 100 # resolution of colorbar
CS = axs.contourf(xseq, yseq, varseq, cbreso, cmap='coolwarm')
q = axs.quiver(xseq.astype(float), yseq.astype(float), varseq1.astype(float)-np.mean(varseq1.astype(float)), varseq2.astype(float)-np.mean(varseq2.astype(float)), scale=10.0)
...
saveName = 'wind' + '_' + str(int(tseq[tind])) +'_contour_nz_' + str(int(zseq[zind])) + '.png'
