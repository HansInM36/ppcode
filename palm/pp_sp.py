...
law53x = np.linspace(1e-2,1e-1,20)
law53seq = np.power(law53x, -5/3)
plt.loglog(law53x, law53seq*1e-6, label='-5/3', linewidth=1.0, color='k', linestyle='--')
# plt.xlim(1e-3,1)
# plt.ylim(1e-20, 1e-2)
plt.xlabel('k_' + axis + ' (m^-1)')
plt.ylabel('S' + varname + ' (' + varunit + ')')
plt.legend()
plt.grid()
plt.title(jobname)
fig.tight_layout() # adjust the layout
saveDir = '/scratch/palmdata/pp/' + jobname + '/'
saveName = varname + '_' + 't' + str(int(tseq[tind])) + '_wsp.png'
plt.savefig(saveDir + saveName)
plt.show()
