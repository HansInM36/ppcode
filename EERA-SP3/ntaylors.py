...
Z = np.array(data.variables['Z'][:], dtype=float)
U = np.array(data.variables['U'][:], dtype=float)
V = np.array(data.variables['V'][:], dtype=float)
THETA = np.array(data.variables['THETA'][:], dtype=float) # it seems that THETA is not potential temperature
...
for zInd in range(zMax):
    ...
    statn = ntaylor.get_statn(u_wrf, u_palm)
    
    fig, ax = plt.subplots(figsize=(4.5,4.5))
    prop = np.asarray((['o', 'k', 'r','15'],['^', 'r', 'r','15'],['*', 'k', 'r','15'],['o', 'k', 'r','15'],['o', 'k', 'r','15']))
    ntaylor.diagn(ax, statn, prop, sigma_lim=1.5, sigma_color='k', \
    crmse_color='C8', r_color='C9', no_sigma_axis=False, \
    no_crmse_axis=False, r_label='correlation coefficient')
    plt.title('H = ' + str(np.round(z_seq[zInd],1)) + 'm')
    saveName = 'ntaylor_H' + str(np.round(z_seq[zInd],1)) + '.png'
    saveDir = '/scratch/projects/EERA-SP3/photo/ntaylor'
