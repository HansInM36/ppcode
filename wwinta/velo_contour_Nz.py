...
# height
zInd = 0
tSeq, xSeq, ySeq, zSeq, varSeq = getData_palm(jobDir, jobName, 'M01', ['.001'], 'u', (0,150), (0,xSeq.size), (0,ySeq.size), (zInd,zInd+1))
...
for tInd in range(0,150,1):
    ...
    plt.title('')
    saveName = "%.4d" % tInd + '.png'
    saveDir = '/scratch/ppcode/wwinta/animation/nowave'
    if not os.path.exists(saveDir):
        os.makedirs(saveDir)
    plt.savefig(saveDir + '/' + saveName, bbox_inches='tight')
    # plt.show()
