...
tSeq, xSeq, ySeq, zSeq, uSeq = getData_palm(jobDir, jobName, 'M02', ['.000','.001','.002','.003'], 'u', [0,42], (0,xSeq.size), (0,ySeq.size), (1,zSeq.size))

t0 = 0

vMin, vMax, vDelta = (-10, -2, 2.0)
cbreso = 100 # resolution of colorbar
levels = np.linspace(vMin, vMax, cbreso + 1)

for tInd in range(0,42,1):
    fig, axs = plt.subplots(figsize=(4.5,3), constrained_layout=False)
    x_ = xSeq
    z_ = zSeq
    v_ = uSeq[tInd,:,0,:]
    #    v_ -= v_.mean()
    ...
    saveDir = '/scratch/projects/WRFnesting/animation/u_Ny'
